...
Run:
  python dexscreener_api.py --token 33eum82LaAhtv5YkUq1BdwEviSErH5CnFxqVNLT5pump --out ./data/raw/analytics --debug

Outputs:
  <out>/<token>/dexscreener/raw/*.json
  <out>/<token>/dexscreener/features.json
  <out>/<token>/dexscreener/summary.txt
"""
...
def build_output_dir(base_out: str | Path, token: str) -> Path:
    return Path(base_out) / token / "dexscreener"
